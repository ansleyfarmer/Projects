...
class BattleshipGame:
    ...
    def __init__(self, num_of_players):
        ...
        if num_of_players == 1:
            ...
            # DONE - IN YOUR SOLUTION, ERASE THE LINE ABOVE AND UNCOMMENT THE LINE BELOW.
            # THEN ADD CODE FOR THE ComputerPlayer.
            # self.players = [HumanPlayer("Player 1"), ComputerPlayer("The Computer")]
        else:
...
class HumanPlayer:
    ...
    def stats(self):
        print("You have attacked " + str(self.attacks) + " times, with " +
              str(self.hits) + " hits and " + str(self.misses) + " misses.")
...
class ComputerPlayer:
    ...
    # Position the fleet randomly.
    def position_fleet(self):
        for boat in self.fleet:
            self.position_boat(boat)

    def position_boat(self, boat):
        orientation = random.choice(["v", "h"])  # Randomly choose orientation
        while True:
    ...
    def take_turn(self):
        ...
        hit_flag = self.opponent.board.attack(x, y)
        if hit_flag:
            print("The computer attacked (" + str(x) + ", " + str(y) + ") and hit a boat!")
            self.hits += 1
        else:
            print("The computer attacked (" + str(x) + ", " + str(y) + ") and missed.")
            self.misses += 1
        self.attacks += 1
